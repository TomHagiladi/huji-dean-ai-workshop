...
.optional>:last-child:nth-child(odd){grid-column:1/-1}
...
STATIONS = [
    ...
    (3, "station-3", "בונים Gem בג'מיני", "בוט זכויות הסטודנט - מודל שפה עם הקשר זהות ותפקיד", None),
    ...
    (6, "station-6", "שאלון אמיתי בגוגל טפסים", "שני מסלולים ליצירת שאלון של היחידה - בלחיצה, ובקוד שאתם מריצים", None),
    (7, "station-7", "ואיך זה נראה אצלנו", "שיחה בזוג, ואז שיחת סיכום משותפת על ההרצאה ועל הסדנה", "end"),
]
...
<div class="callout"><b>המטרה של התחנה:</b> להשתמש בחופשיות במגוון הכלים שעומדים לרשותכם כדי לקדם את המטרות של היחידה. בתחנות הבאות הדגש הוא על כלי אחד; כאן הבחירה שלכם - עוברים בין הכלים, משווים ביניהם, ומצרפים אותם זה לזה. אפשר גם להרים יד ולהתייעץ עם תום.</div>
<div class="callout"><b>אפשר גם פרפלקסיטי:</b> <a href="https://www.perplexity.ai" target="_blank" rel="noopener"><span class="en">Perplexity</span></a> הוא מנוע חיפוש מבוסס בינה מלאכותית. במקום להחזיר רשימת קישורים הוא מחפש ברשת, קורא את המקורות, ומחזיר תשובה כתובה עם הפניה ממוספרת לכל טענה - כך שאפשר לבדוק מאיפה כל דבר הגיע. שימושי במיוחד לשאלות שדורשות מידע עדכני או השוואה למה שקורה בעולם.</div>
<div class="callout"><b>אפשר גם מתוך מחברת:</b> העלו ל-<a href="https://notebook.google.com" target="_blank" rel="noopener"><span class="en">Gemini Notebook</span></a> את החומרים של היחידה - מסמך המטרות מאתמול, נהלים, מצגות, סיכומי ישיבות - ונהלו את השיחה בתוך המחברת עצמה, כך שהתשובות נשענות על החומרים שלכם. אפשר גם לבקש ממנה תוצר: מפת חשיבה, למשל, פורשת את המטרות ואת הקשרים ביניהן ועוזרת לראות מה באמת עומד על הפרק. לכלי הזה יש <a href="station-2.html">תחנה ייעודית בהמשך</a>.</div>
...
{prompt("המשך - פרקליט השטן", "עכשיו תהיה פרקליט השטן. מה בתוכנית הזו ייכשל, ולמה? התייחס לכל שלב בנפרד.")}
...
<li>בחלונית ה-<span class="en">Studio</span> מייצרים כמה תוצרים שונים מאותם מקורות: <span class="en">Audio Overview</span>, <span class="en">Mind Map</span>, <span class="en">Video Overview</span>, ומה שעוד מופיע שם</li>
</ol>
<p><b>שלוש שאלות לדוגמה - כל אחת מועתקת בנפרד:</b></p>
{prompt("שאלה 1", "סטודנט במילואים פספס מועד הגשה. מה מגיע לו, ולפי איזה סעיף?")}
{prompt("שאלה 2", "הכן לי רשימה של עשר השאלות הנפוצות ביותר שסטודנט חדש ישאל על הנהלים האלה, עם תשובה קצרה לכל אחת.")}
{prompt("שאלה 3", "אילו זכויות מופיעות בתקנון אבל כמעט לא מוזכרות בטפסים?")}
<div class="callout"><b>לפני השאלה - ייצרו מגוון תוצרים:</b> לא רק פודקאסט. מאותם מקורות בדיוק אפשר לייצר גם מפת חשיבה, סרטון, מצגת, דוח וכרטיסיות תרגול - כל מה שמופיע אצלכם בחלונית. כל תוצר מספר את אותו חומר בצורה אחרת, ולקהל אחר.</div>
<div class="callout"><b>שאלה לזוג:</b> הקשיבו לדקה מהפודקאסט ועברו על שאר התוצרים. למי ביחידה, או לאילו סטודנטים, זה היה חוסך זמן אמיתי?</div>
'''

BODY[3] = f'''
<p class="lead">בונים בוט (מודל שפה עם הקשר זהות ותפקיד) שעונה על שאלות בנושא זכויות הסטודנט. מי שבנה מחברת בתחנה 2 יכול לצרף אותה כידע ל-Gem בלחיצה אחת.</p>
<ol class="steps">
...
<div class="callout"><b>וזו רק המדרגה הראשונה:</b> היום, בעזרת וויב קודינג, אפשר לייצר בקלות לא רק בוט אלא פלטפורמה דיגיטלית שלמה - עם ממשק משתמש נוח, גמישות גבוהה הרבה יותר, וחוויית משתמש מתוחכמת ונעימה מזו של הבוט המובנה בג'מיני או ב-<span class="en">ChatGPT</span>. <a href="station-5.html">בתחנה 5</a> טועמים את זה.</div>
...
<div class="callout"><b>שאלה נוספת לזוג:</b> האם הסטודנטים שלכם יודעים להשתמש כך בצ'אט? ואם היו יודעים שאפשר - האם יש להם האוריינות השפתית והיכולת הקוגניטיבית לנסח פרומפט כמו אלה שכאן, או להיעזר בכלי ככלי תומך בהפרעות קשב ובקשיים אחרים?</div>
'''
...
<div class="callout"><b>מה חשוב לדעת:</b> הכלי הזה בג'מיני מוגבל ביכולות שלו - הוא טעימה. אבל <span class="en">ChatGPT Work</span>, <span class="en">Claude Code</span>, <span class="en">Google Antigravity</span>, <span class="en">Grok Bot</span> ועוד שחקנים בשוק מאפשרים היום לבנות אפליקציות ייעודיות בקלות - כאלה שמשרתות את צוות היחידה או את הסטודנטים, עם נתונים אמיתיים ועם תחזוקה לאורך זמן.</div>
<div class="callout"><b>שאלה לזוג:</b> מה הדבר הראשון שהאפליקציה שלכם עשתה נכון, ומה היא לא הבינה? מה בניסוח שלכם גרם לזה?</div>
<div class="callout"><b>שאלה נוספת לזוג:</b> אילו יכולתם לפתח כל אפליקציה שהיא ליחידה שלכם - אילו אפליקציות הייתם מפתחים היום? ומי היה קהל היעד שלהן: צוות היחידה או הסטודנטים?</div>
'''

BODY[6] = f'''
<p class="lead">הטופס הוא כנראה הכלי הכי נפוץ בדיקנט, ובדרך כלל בונים אותו ביד - שאלה אחרי שאלה. כאן תבנו שאלון אמיתי של היחידה שלכם בשני מסלולים, ותראו מתי כל אחד מהם עדיף.</p>
<div class="callout"><b>הרציונל:</b> במסלול הראשון <span class="en">Gemini</span> מנסח לכם את השאלון בתוך גוגל טפסים. במסלול השני אתם מבקשים ממודל שפה קוד קצר, מריצים אותו, והטופס נבנה בדיוק לפי מפרט. הרעיון הגדול כאן הוא לא הטופס: זו היכולת לייצר כל תוצר של גוגל - טופס, גיליון, מסמך, מצגת - דרך קוד שביקשתם, גם בלי לדעת לתכנת.</div>

<p><b>שלב 1 - בוחרים שאלון אמיתי, לא תרגיל.</b> משהו שהיחידה שלכם באמת צריכה:</p>
<ul class="tips">
<li><b>מעונות:</b> שביעות רצון מהמגורים בסוף סמסטר, כולל תחזוקה וחיי קהילה</li>
<li><b>סיוע כלכלי:</b> משוב מסטודנטים שקיבלו מלגה - מה הקלה, ומה עדיין חסר</li>
<li><b>שוויון הזדמנויות:</b> שאלון איתור מוקדם, שמזהה סטודנט שמתחיל להתרחק לפני שהוא נושר</li>
<li><b>נגישות ולקויות למידה:</b> משוב על ההתאמות שניתנו - מה עבד בפועל, ומה נשאר על הנייר</li>
<li><b>מרכז הקריירה:</b> סקר צרכים לקראת בניית סדנאות - מה הסטודנטים באמת מבקשים</li>
<li><b>השירות הפסיכולוגי:</b> טופס פנייה ראשוני, עם שאלות שעוזרות לתעדף</li>
</ul>
{prompt("שלב 2 - מנסחים את השאלות", "אני [תפקיד] ביחידת [שם היחידה] בדיקנט הסטודנטים באוניברסיטה העברית. אני רוצה לבנות שאלון [משוב / איתור / סקר צרכים] שיישלח ל[מי הנמענים], במטרה [מה אני רוצה לדעת, ומה אעשה עם התשובות]. נסח לי שאלון בעברית שכולל [מספר] שאלות: חלקן סגורות בסולם 1 עד 5, חלקן בחירה מרובה, ושתיים או שלוש פתוחות. הוסף כותרת וטקסט פתיחה קצר שמסביר לנמענים למה השאלון חשוב וכמה זמן הוא ייקח. אחרי שתציע - הצע לי שלוש שאלות נוספות שלא חשבתי עליהן, והסבר במשפט אחד למה כל אחת חשובה.", BASIS)}

<p><b>שלב 3 - מסלול א': השאלון נכתב בלחיצה.</b></p>
<ol class="steps">
<li>פותחים טופס חדש ב-<a href="https://forms.google.com" target="_blank" rel="noopener"><span class="en">forms.google.com</span></a></li>
<li>לוחצים על <span class="en">Help me create a form</span> - בחלון שנפתח בטופס ריק, או בכפתור שבסרגל הכלים</li>
<li>מדביקים את התיאור של השאלון, לוחצים <span class="en">Create</span>, עוברים על הטיוטה, ומאשרים ב-<span class="en">Create form</span></li>
<li>לא אהבתם? אפשר לערוך את הבקשה שלמעלה ולשלוח שוב, או ללחוץ <span class="en">Recreate</span></li>
</ol>
<div class="callout"><b>מה המסלול הזה עדיין לא עושה:</b> גוגל מציינת שהיצירה עם <span class="en">Gemini</span> אינה בונה טפסים מרובי מקטעים, ואינה עורכת טופס קיים - כל יצירה חדשה מחליפה את הקודמת. אם הכפתור לא מופיע אצלכם, עברו למסלול השני. העברית נתמכת מאפריל 2026.</div>

<p><b>שלב 4 - מסלול ב': אתם מריצים קוד.</b> מתחילים מאפס: אין טופס ואין כלום - יש רק את השאלות משלב 2, והקוד יוצר את הטופס בעצמו. כך מקבלים שליטה מלאה - מקטעים, סוגי שאלות מדויקים, ושאלון שאפשר לשחזר בדיוק כמו שהוא בשנה הבאה.</p>
{prompt("הפרומפט לקוד", "כתוב לי קוד Google Apps Script שיוצר טופס חדש בגוגל טפסים. אין לי טופס קיים - הקוד צריך לייצר אותו מאפס בעזרת FormApp.create. אלה השאלות: [הדביקו כאן את השאלות המאושרות משלב 2]. ודא שהקוד עושה את כל אלה: נותן לטופס כותרת וטקסט פתיחה; מגדיר לכל שאלה את הסוג המתאים - סולם, בחירה מרובה, תיבות סימון, תשובה קצרה או תשובה ארוכה; מסמן אילו שאלות הן חובה; ובסוף מדפיס עם Logger.log גם את קישור העריכה של הטופס וגם את קישור המילוי שלו. הוסף מעל כל קטע בקוד הערה קצרה בעברית שמסבירה מה הוא עושה.", "בסיס לעריכה. רוצים שאלון מרובה מקטעים - מה שהמסלול הראשון לא יודע לעשות? הוסיפו לפרומפט: חלק את השאלון למקטעים לפי נושא.")}
<ol class="steps">
<li>נכנסים ל-<a href="https://script.google.com" target="_blank" rel="noopener"><span class="en">script.google.com</span></a> ולוחצים <span class="en">New project</span>, פרויקט חדש</li>
<li>מוחקים את קוד הדוגמה שמופיע שם, מדביקים את הקוד שקיבלתם, ושומרים</li>
<li>לוחצים <span class="en">Run</span></li>
<li>בפעם הראשונה תתבקשו לאשר הרשאות: <span class="en">Review permissions</span>, בוחרים חשבון, <span class="en">Advanced</span>, ואז <span class="en">Go to</span> עם שם הפרויקט, ולסיום <span class="en">Allow</span></li>
<li>בתחתית המסך נפתח יומן הריצה, <span class="en">Execution log</span>, ובו שני הקישורים שהקוד הדפיס - אחד לעריכת הטופס ואחד למילוי שלו</li>
<li>לוחצים על קישור העריכה: הטופס כבר שם, בנוי במלואו ושמור בדרייב שלכם</li>
</ol>
<div class="callout"><b>קיבלתם שגיאה? זה חלק מהתהליך.</b> העתיקו את הודעת השגיאה במלואה, הדביקו אותה בחזרה למודל, ובקשו תיקון. הריצו שוב. שני סיבובים כאלה הם דבר רגיל לגמרי - זה בדיוק מה שמתכנת עושה.</div>
<div class="callout"><b>אבטחת מידע:</b> את השאלות מנסחים מול המודל בלי שמות של סטודנטים ובלי פרטים מזהים. הטופס עצמו והתשובות שיגיעו אליו נשארים בחשבון הגוגל שלכם.</div>

<p><b>ומכאן זה נפתח:</b> אותה טכניקה בדיוק - לבקש קוד, להריץ, לתקן - עושה גם את אלה:</p>
<ul class="tips">
<li>מייל אישור אוטומטי לכל מי שמילא את הטופס</li>
<li>ריכוז התשובות בגיליון, עם ניתוח וגרפים שנבנים לבד</li>
<li>הפקת מסמכים מתבנית - אישורים, מכתבים, זימונים</li>
<li>אירועים ביומן מתוך טבלה, למשל תורנויות או פגישות</li>
<li>התראה במייל כשמגיעה פנייה שדורשת טיפול מיידי</li>
</ul>
<div class="callout"><b>שאלה לזוג:</b> איזה טופס ביחידה שלכם ממלאים היום על נייר, או שלא ממלאים בכלל כי לבנות אותו לוקח יותר מדי זמן? ומה הייתם עושים עם התשובות אם הן היו מגיעות מסודרות?</div>
'''

BODY[7] = '''
<p class="lead">עוצרים את ההתנסות ומדברים בזוג על מה שראיתם היום, ועל מה שאפשר לעשות איתו ביחידה שלכם.</p>
<ol class="steps">
...
<div class="callout"><b>ואז מתכנסים:</b> בתום השיחה בזוגות נתכנס יחד לשיחת סיכום על כל מה שעברנו - ההרצאה והסדנה.</div>
'''
...
<div class="wrap">
  ...
  <div class="grid2">
    <section class="card tone" aria-labelledby="how-h">
      ...
      <p class="lead">בזוגות, מחשב אחד לזוג. המטרה היא לעשות יחד, לדבר על ההתנסות תוך כדי, ולקיים סיעור מוחות בין שני אנשים לבין הבינה המלאכותית - ולדון בחוויה.</p>
  ...
  <section class="route" id="route" aria-labelledby="route-h">
    ...
    <p class="hint">כולם מתחילים בתחנה 1. אחרי שמיציתם אותה - בחירה חופשית בין תחנות 2 עד 6, כל זוג לפי המסלול והקצב שלו. כולם מסיימים בתחנה 7.</p>
    {stcard(*s[1])}
    <div class="group-title"><h3>בחירה חופשית - אחרי תחנה 1</h3></div>
    <div class="optional">{stcard(*s[2])}{stcard(*s[3])}{stcard(*s[4])}{stcard(*s[5])}{stcard(*s[6])}</div>
    {stcard(*s[7])}
  </section>
</div>
</main>
...
def station(num, slug, title, what, kind):
    ...
    for (n2, slug2, t2, w2, k2) in STATIONS:
        ...
        if n2 < len(STATIONS): steps += '<li class="ln" aria-hidden="true"></li>'
